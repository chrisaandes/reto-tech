...
export const responseBodyMiddleware: RequestHandler = (_req, res, next) => {
...
export const customLogLevel = (_req: IncomingMessage, res: ServerResponse<IncomingMessage>, err?: Error): LevelWithSilent => {
...
export const customSuccessMessage = (req: IncomingMessage, res: ServerResponse<IncomingMessage>) => {
  if (res.statusCode === StatusCodes.NOT_FOUND) return getReasonPhrase(StatusCodes.NOT_FOUND);
  return `${req.method} completed`;
};

export const genReqId = (req: IncomingMessage, res: ServerResponse<IncomingMessage>) => {
